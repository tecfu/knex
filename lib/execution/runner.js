...
class Runner {
  ...
  stream(optionsOrHandler, handlerOrNil) {
    ...
    const stream = new Transform({
      ...
      transform: (chunk, _, callback) => {
        try {
          // For streams the raw driver chunk is the same as the processed value.
          callback(
            null,
            this.client.postProcessResponse(chunk, queryContext, {
              driverResponse: chunk,
              runner: this,
            })
          );
        } catch (err) {
  ...
  async query(obj) {
    ...
    return queryPromise
      .then((resp) => {
        // Preserve the raw driver response so postProcessResponse can access it
        // via the optional third argument if needed.
        const driverResponse = resp;
        const processed = this.client.processResponse(resp, runner);
        // processResponse may return a Promise (e.g. sqlite3 DDL) or a value.
        return Promise.resolve(processed).then((processedResponse) => ({
          processedResponse,
          driverResponse,
        }));
      })
      .then(({ processedResponse, driverResponse }) => {
        const postProcessedResponse = this.client.postProcessResponse(
          processedResponse,
          queryContext,
          { driverResponse, runner }
        );
